  ...
  return (
    <Link
      ...
      className={[
        ...
        active
          ? "bg-[var(--coral)] text-white border-black/10"
          : "bg-white hover:bg-[var(--rose-100)] border-black/10",
      ].join(" ")}
    >
      {label}
    </Link>
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-40 bg-[var(--bg)]/90 backdrop-blur border-b border-black/10">
        <div className="mx-auto max-w-6xl px-4 py-4 flex items-center justify-between gap-4">
          <div>
            <div className="font-[var(--font-display)] text-xl leading-none">
              Psi • Maria Laura
            </div>
          ...
          <nav className="flex flex-wrap items-center gap-2 justify-end">
            ...
            <button
              onClick={sair}
              className="px-4 py-2 rounded-full border-2 border-black/10 bg-[var(--butter)] font-semibold text-sm"
            >
              Sair
            </button>
          </nav>
